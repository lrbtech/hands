...
class AddServiceComponent extends StatefulWidget {
  ...
  AddServiceComponent({Key? key, this.isFromService = false, this.addressData})
      : super(key: key);
  ...
  Future<void> addUpdateAddress() async {
    ...
    if (formKey.currentState!.validate()) {
      ...
      await locationFromAddress(addressNameCont.text).then((value) {
        ...
        request.putIfAbsent(
            AddAddressKey.longitude, () => value.first.longitude);
      }).catchError((e) async {
      ...
      if (!request.containsKey(AddAddressKey.latitude)) {
        ...
        request.putIfAbsent(AddAddressKey.id,
            () => isUpdate ? widget.addressData!.id.validate() : "");
