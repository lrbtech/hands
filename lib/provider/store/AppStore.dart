...
import 'package:hands_user_app/provider/locale/applocalizations.dart';
...
class AppStorePro = _AppStore with _$AppStore;
...
abstract class _AppStore with Store {
  ...
  void enableTracking(int trackingBookingId) {
    trackingJobId = trackingBookingId;
  ...
  @action
  Future<void> setSignupAddress(AddressModel newSignupAddress) async {
    signupAddress = newSignupAddress;
  }
  ...
  Future<void> setCategoriesIDs(List<String> val,
      {bool isInitializing = false}) async {
    categoriesIDs = val;
    if (!isInitializing) await setValue(CATEGORIES_IDS, val);
  }

  @action
  Future<void> setCategoryId(int val, {bool isInitializing = false}) async {
    categoryID = val;
    if (!isInitializing) await setValue(CATEGORY_ID, val);
  }
  ...
  @action
  Future<void> setSellerAgreement(String val,
      {bool isInitializing = false}) async {
    sellerAgreement = val;
    if (!isInitializing)
      await compareValuesInSharedPreference(SELLER_AGREEMENT, val);
  }
