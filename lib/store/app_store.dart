...
abstract class _AppStore with Store {
  ...
  @computed
  bool get isArabic => selectedLanguageCode == 'ar';
  ...
  void setStoresUrls({required SiteConfig config}) {
    appstoreUrl = config.appstoreUrl.validate();
    providerAppstoreUrl = config.providerAppstoreUrl.validate();
    playStoreUrl = config.playstoreUrl.validate();
    providerPlayStoreUrl = config.providerPlaystoreUrl.validate();
  }
  ...
  Future<void> setFirebaseToken(String val, {bool isInitializing = false}) async {
    firebaseToken = val;
    if (!isInitializing) await setValue(FIREBASE_TOKEN, val);
  }
